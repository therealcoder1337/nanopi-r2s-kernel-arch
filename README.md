...
The package installs `/boot/Image`, the R2S DTB, a mkinitcpio preset/config, and modules under `/usr/lib/modules/<kernel-release>/`. A pacman hook regenerates the zstd initramfs; other initramfs compression formats are intentionally unsupported.
...
It removes desktop/media/wireless/virtualization support, SATA/AHCI, USB gadget mode, and non-R2S SoC and device drivers. Legacy filesystems, partition parsers, crypto/netfilter helpers, niche routing and virtual networking, optional storage layers, and debug/test code are also trimmed.
